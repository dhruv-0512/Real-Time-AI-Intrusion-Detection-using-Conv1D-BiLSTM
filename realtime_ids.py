...
USAGE
  ...
  python realtime_ids.py --iface 4               # Windows interface index
...
# ============================================================================
CONFIG = dict(
    model_path   = os.path.join("models", "fixed_model.keras") if os.path.exists(os.path.join("models", "fixed_model.keras")) else (os.path.join("models", "bilstm_ids.h5") if os.path.exists(os.path.join("models", "bilstm_ids.h5")) else "bilstm_ids.h5"),
    columns_path = os.path.join("models", "columns.pkl") if os.path.exists(os.path.join("models", "columns.pkl")) else "columns.pkl",
    interface    = "4",                   # tshark interface index or name
    log_file     = os.path.join("logs", "ids_log.txt"),
    alert_file   = os.path.join("logs", "ids_alerts.jsonl"),   # one JSON object per alert line
...
def compute_severity(label: str, confidence: float) -> str:
    """Map prediction to SOC-style severity level."""
    if label == "NORMAL":
        return "INFO"
    if confidence > 0.85:
...
class ConfirmationBuffer:
    """Temporal confirmation: only alert if a pattern persists across N predictions.
    Reduces false positives by requiring consistent classification."""
    def __init__(self, window_size=5, threshold=3):
        self.window_size = window_size
        self.threshold = threshold
        self._history = deque(maxlen=window_size)
    
    def add(self, label: str, confidence: float) -> bool:
        """Add prediction; return True if label is confirmed (persists)."""
        self._history.append(label)
        if label == "NORMAL":
            return True  # normal always passes
        count = sum(1 for h in self._history if h == label)
...
class AlertDeduplicator:
    """Groups repeated alerts from the same source into single events."""
    def __init__(self, merge_window_sec=30):
        self.merge_window = merge_window_sec
        self._active = {}  # key: (label, src_ip) -> {first_seen, last_seen, count, ...}
    
    def process(self, payload: dict) -> dict:
        """Returns enriched payload with dedup fields. Updates active alerts."""
        key = (payload.get("label", ""), payload.get("src_ip", ""))
        now = datetime.now()
        
        if key in self._active:
            entry = self._active[key]
            elapsed = (now - entry["_last"]).total_seconds()
            if elapsed < self.merge_window:
                entry["occurrences"] += 1
                entry["last_seen"] = now.isoformat()
                entry["_last"] = now
                entry["confidence"] = max(entry["confidence"], payload.get("confidence", 0))
                payload.update({
...
class PacketStats:
    """Track real-time packet throughput and protocol distribution."""
    def __init__(self):
        self.total_packets = 0
        self.total_bytes = 0
        self.proto_counts = defaultdict(int)  # tcp, udp, icmp
        self._start_time = time.time()
        self._recent_times = deque(maxlen=100)  # for packets/sec calc
    
    def record(self, raw_fields: dict):
        self.total_packets += 1
        try:
            self.total_bytes += int(float(raw_fields.get("frame_len", 0)))
        except (ValueError, TypeError):
            pass
        proto = proto_to_kdd(raw_fields.get("ip_proto", "6"))
    ...
    def packets_per_sec(self) -> float:
        ...
        return len(self._recent_times) / span if span > 0 else 0.0
    ...
    def to_dict(self) -> dict:
        return {
            ...
            "bytes_per_sec": round(self.total_bytes / max(self.uptime_sec, 1), 0),
            "uptime_sec": round(self.uptime_sec, 0),
            "protocol_breakdown": dict(self.proto_counts),
        }
...
def print_prediction(label: str, conf: float, pkt_count: int,
                     low_conf: bool, logger, severity: str = "INFO",
                     confirmed: bool = True):
    ...
    cf     = "" if confirmed else " (unconfirmed)"
    sev_badge = f"{sev_c}[{severity}]{Style.RESET_ALL}"
    line   = f"  Pkts:{pkt_count:>6}  {sev_badge} {color}{badge}{Style.RESET_ALL}  conf={conf_s}{lc}{cf}"
    print(line)

    if label != "NORMAL" and confirmed:
        alert_color = LABEL_COLOR.get(label, Fore.RED)
        print(alert_color +
              f"  +== !  ALERT: {severity} - {label} DETECTED  (conf={conf_s}) ==+" +
              Style.RESET_ALL)
    STATS[label] += 1
...
def run_ids(cfg: dict):
    ...
    confirmation = ConfirmationBuffer(window_size=5, threshold=3)
    deduplicator = AlertDeduplicator(merge_window_sec=30)
    ...
    buffer    = []
    pkt_count = 0
    last_raw  = {}  # track last packet's raw fields for IP info
        ...
        while True:
            ...
            if len(buffer) >= WINDOW_SIZE:
                ...
                try:
                    ...
                    confirmed = confirmation.add(label, conf)
                    severity  = compute_severity(label, conf)

                    print_prediction(label, conf, pkt_count, low_conf, logger,
                                     severity=severity, confirmed=confirmed)
                    logger.debug("Probs: %s", dict(zip(LABELS, preds.round(3).tolist())))

                    # -- Write alert (all events, with severity) ---
                    src_ip   = last_raw.get("src_ip", "0.0.0.0")
                    dst_ip   = last_raw.get("dst_ip", "0.0.0.0")
                    protocol = proto_to_kdd(last_raw.get("ip_proto", "6")).upper()
                    src_port = last_raw.get("src_port", "0")
                    dst_port = last_raw.get("dst_port", "0")
                    ...
                    # -- Deduplication -----------------------------
                    payload = deduplicator.process(payload)

                    # Always write every prediction so the dashboard has data
                    write_alert(cfg["alert_file"], payload)

                    if label != "NORMAL" and confirmed:
                        logger.warning(
                            "ALERT %s [%s] conf=%.2f%% src=%s dst=%s",
                            label,
                            severity,
                            conf * 100,
                            src_ip,
                        ...
                        logger.info(
                            "Suppressed unconfirmed %s (conf=%.1f%%)",
                            label,
                            conf * 100,
                        )
                except Exception as e:
...
def main():
    ...
    ap.add_argument("--demo",        type=str, default=None, metavar="PCAP",
                    help="Demo mode: replay a .pcap file instead of live capture")
    args = ap.parse_args()

    if args.list_ifaces:
        list_interfaces(args.tshark)

    # -- Demo mode: delegate to pcap_replay ------------------------------
    if args.demo:
        print(Fore.CYAN + "\n  === DEMO MODE ===  Replaying: " + args.demo + Style.RESET_ALL + "\n")
        import pcap_replay
        sys.argv = ["pcap_replay.py",
                    "--pcap", args.demo,
                    "--model", args.model,
                    "--columns", args.columns,
                    "--alert-file", args.alerts,
                    "--log-file", args.log]
        pcap_replay.main()
        return
